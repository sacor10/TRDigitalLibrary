...
records and text from the Library of Congress, indexes them in SQLite FTS5, exposes a
...
> **Production note:** do not run ingestion from the live server process. Import
> during CI/build, then ship the generated `data/library.db` with the deployment
> artifact. The Netlify build follows this pattern by running
> `npm run ingest-loc -- --limit 25 --reset` before `npm run build`.
...
TRDigitalLibrary/
├── data/
│   └── library.db           # generated by `npm run ingest-loc` (gitignored)
